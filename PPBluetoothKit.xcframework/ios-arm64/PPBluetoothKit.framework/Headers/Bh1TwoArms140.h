...
@property(nonatomic, assign) unsigned int         bhProduct;     //!< 0(默認算法)  1~255(定製化算法)
@property(nonatomic, assign) float                bhTrimming;    //!< 微調體成分結果，1默認標準算法 越大體脂率越低,悠健不保證微調後的數據，對使用微調後的結果分析也不提供相關FAE,僅開放給客戶自行驗證測試
@property (nonatomic,assign) unsigned int         bhZTwoArmsEnCode;    //!< 双手阻抗加密值(下位机上传值) 范围0 ~ 0xFFFFFF
...
@property (nonatomic,readonly,assign) float         bhZTwoArmsDeCode;   //!< 双手阻抗值(Ω)  200 ~ 1200 for debug 通常不显示
